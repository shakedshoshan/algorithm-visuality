        ...
        >
          {activeTab === 'implementation' ? (
            ...
            <div className="bg-white p-6 rounded-lg shadow-md max-w-5xl mx-auto">
              ...
              <div className="mb-6">
                <h4 className="text-lg font-semibold text-indigo-600 mb-2">Example 1: Weather API Adapter in React</h4>
                <p className="text-gray-700 mb-4">
                  In a React application, you might need to integrate with multiple weather APIs that return data in different formats. 
                  Here's how you could implement adapters for these APIs:
                </p>
                <div className="bg-gray-50 p-4 rounded border text-black border-gray-200">
                  <pre className="text-sm overflow-auto">
                    <code className="language-javascript text-black">
{`// Expected data format for our weather component
type WeatherData = {
...
// First API response format
type API1Response = {
  temp: { value: number, unit: string };
  humidity: string;  // "60%"
  wind: { speed: string };  // "12 km/h"
};

// Second API response format
type API2Response = {
...
// Adapter for API 1
const adaptWeatherDataFromAPI1 = (apiData: API1Response): WeatherData => ({
  temperature: apiData.temp.value,
  humidity: parseInt(apiData.humidity),
  windSpeed: parseInt(apiData.wind.speed)
});

// Adapter for API 2
const adaptWeatherDataFromAPI2 = (apiData: API2Response): WeatherData => ({
  temperature: apiData.temperatureCelsius,
  humidity: apiData.relativeHumidity,
  windSpeed: apiData.windKph
});

// Weather component that consumes the adapted data
...
const WeatherDashboard = () => {
  // Fetch from API 1 and adapt the data
  const api1Data = fetchFromAPI1();
  const adaptedAPI1Data = adaptWeatherDataFromAPI1(api1Data);
  
  // Fetch from API 2 and adapt the data
  const api2Data = fetchFromAPI2();
  const adaptedAPI2Data = adaptWeatherDataFromAPI2(api2Data);
  
  return (
    <div>
      <h1>Weather Dashboard</h1>
      <WeatherDisplay weatherData={adaptedAPI1Data} />
    </div>
  );
};`}
                    </code>
              ...
              <div className="mb-6">
                <h4 className="text-lg font-semibold text-indigo-600 mb-2">Example 2: Analytics Adapter</h4>
                <p className="text-gray-700 mb-4">
                  Analytics adapters are common in web applications where you might want to use a consistent interface to track events across multiple analytics providers:
                </p>
...
// Adaptee 1: Google Analytics
class GoogleAnalyticsAdapter implements AnalyticsAdapter {
  trackEvent(eventName: string, properties?: Record<string, any>): void {
    // Adapt to Google Analytics format
...
class MixpanelAdapter implements AnalyticsAdapter {
  trackEvent(eventName: string, properties?: Record<string, any>): void {
    // Adapt to Mixpanel format
    window.mixpanel.track(eventName, properties);
  }
  
  trackPageView(pageName: string): void {
    window.mixpanel.track('page_view', { page: pageName });
  }
}

// Composite adapter that sends to multiple analytics services
class CompositeAnalyticsAdapter implements AnalyticsAdapter {
  ...
  trackEvent(eventName: string, properties?: Record<string, any>): void {
    this.adapters.forEach(adapter => adapter.trackEvent(eventName, properties));
  }
  
  trackPageView(pageName: string): void {
    this.adapters.forEach(adapter => adapter.trackPageView(pageName));
  }
}
...
const SignupButton = () => {
  ...
  const handleSignup = () => {
    analytics.trackEvent('signup_click');
    // Signup logic
  };
